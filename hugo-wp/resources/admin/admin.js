...
(function ($) {
    ...
    function disableNotice() {
        wp.ajax.post('hugo_wp_disable_big_notice', {
            nonce: hugo_wp_builder_status.hugo_wp_disable_big_notice_nonce
        });
    }
    ...
    function activateBuilder(callback) {
        ...
        wp.ajax.post('hugo_wp_activate_plugin', {
            slug: hugo_wp_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: hugo_wp_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.hugo-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('hugo_wp_front_set_predesign', {
            index: selectedFrontPage,
            nonce: hugo_wp_builder_status.hugo_wp_front_set_predesign_nonce
        });
